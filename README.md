...
- 172 testes automatizados validados localmente
...
Dois arquivos merecem destaque:

- `tests/test_regressions.py` — trava os bugs já corrigidos (porta 443, timeout de
  WHOIS, repetição de questões do quiz, sufixos compostos tipo `.com.br`, rate limit).
  Cada teste ali falha na versão anterior à correção.
- `tests/test_uncovered_modules.py` — cobre os módulos que antes não tinham teste
  algum: downloader, logger, estado, tema, widgets e workers.
