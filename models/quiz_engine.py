...
class QuizQuestion:
    ...
    source_id: str = ""           # id da questão-modelo no banco (evita repetir na rodada)
...
class QuizEngine:
    ...
    def __init__(self):
        ...
        # Ids de origem já servidos na rodada atual (evita repetir pergunta).
        self._served_source_ids: set[str] = set()
    ...
    def generate_question(self, difficulty: str = "iniciante") -> QuizQuestion:
        ...
        # Filtra questões do nível que ainda não foram servidas nesta rodada.
        # Compara contra os ids DE ORIGEM (estáticos, tipo "init_01"); comparar
        # contra _active_questions não funcionaria, pois lá as chaves são uuid4.
        same_difficulty = [q for q in self._questions_bank if q.difficulty == difficulty]
        available = [
            q for q in same_difficulty
            if q.question_id not in self._served_source_ids
        ]

        if not available and same_difficulty:
            # Banco do nível esgotado: reinicia o ciclo para esta dificuldade.
            self._served_source_ids -= {q.question_id for q in same_difficulty}
            available = same_difficulty

        if not available:
            available = [
                q for q in self._questions_bank
                if q.question_id not in self._served_source_ids
            ] or self._questions_bank

        question = random.choice(available)
        self._served_source_ids.add(question.question_id)
    ...
    def reset_statistics(self):
        """Reseta estatísticas do usuário e inicia uma nova rodada."""
        self._stats = UserStats()
        # Início de rodada: libera todas as questões para serem servidas de novo.
        self._served_source_ids.clear()
    ...
    def _build_question_bank(self):
        ...
        self._questions_bank.extend([
            QuizQuestion(
                ...
                question_text="Examine a URL abaixo. Esta URL é SEGURA ou MALICIOSA?",
    ...
    def add_questions_from_dataset(self, urls_malicious: list[str],
        ...
        for url in urls_malicious[:20]:
            self._questions_bank.append(QuizQuestion(
                ...
                url_defanged=url,
                question_text="Examine a URL abaixo. Esta URL é SEGURA ou MALICIOSA?",
                url_display=URLDefanger.refang(url),
                correct_answer=False,
            ...
            for url in mal_sample:
                ...
                self._questions_bank.append(QuizQuestion(
                    ...
                    explanation=(
                        ...
                        "Examine o domínio, protocolo e path para identificar os sinais."
                    ),
