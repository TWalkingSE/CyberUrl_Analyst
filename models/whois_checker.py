...
class WhoisChecker:
    ...
    def check_domain_age(self, domain: str) -> WhoisResult:
        ...
        try:
            # NÃO usar `with`: o __exit__ do executor chama shutdown(wait=True),
            # que bloquearia até a consulta lenta terminar — anulando o timeout.
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        "WHOIS timeout para %s após %ds",
                        domain,
                        WHOIS_TIMEOUT_SECONDS,
                    )
                    # Desliga sem esperar: a thread órfã morre com o processo.
                    executor.shutdown(wait=False, cancel_futures=True)
                    return WhoisResult(
                        domain=domain,
                        error=(
                            "Consulta WHOIS excedeu o tempo limite "
                            f"({WHOIS_TIMEOUT_SECONDS}s)."
                        ),
                    )
