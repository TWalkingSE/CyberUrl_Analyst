...
class TestDatasetMemoryLimit(unittest.TestCase):
    def test_max_entries_constant_is_sane(self):
        ...
        self.assertIsInstance(DATASET_MAX_ENTRIES, int)
        self.assertEqual(DATASET_MAX_ENTRIES, 2_000_000)

    def test_entries_above_cap_are_not_stored_nor_counted(self):
        """
        Acima do teto nada é armazenado — e a contagem devolvida precisa
        refletir isso, senão o painel informa mais itens do que é possível
        consultar de fato.
        """
        from models import dataset_manager as dm

        manager = dm.DatasetManager.__new__(dm.DatasetManager)
        manager._malicious_urls = set()
        manager._malicious_domains = set()
        manager._legitimate_domains = set()
        manager._dga_domains = set()
        manager._dataset_urls = {"t": set()}
        manager._dataset_domains = {"t": set()}

        with mock.patch.object(dm, "DATASET_MAX_ENTRIES", 2):
            stored = [
                manager._add_url_entry("t", f"http://exemplo{i}.com/a")
                for i in range(5)
            ]

        self.assertEqual(stored, [True, True, False, False, False])
        self.assertEqual(len(manager._malicious_urls), 2)
