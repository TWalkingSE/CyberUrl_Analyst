"""
Página Glossário Interativo — Referência de termos de cibersegurança.

Termos organizados por categoria com busca, exemplos reais e
...
GLOSSARY: list[dict] = [
    ...
    # ── Técnicas de Manipulação de URL ───────────────────────────────
    {
        "term": "Typosquatting",
        "category": "URLs",
        "definition": (
...
GLOSSARY_CATEGORIES = sorted(set(t["category"] for t in GLOSSARY))


def page_glossary():
    """Renderiza a página do glossário interativo."""
    unlock_badge("glossary_explorer")
    st.title(f"📖 {T('nav.glossary') if T('nav.glossary') != 'nav.glossary' else 'Glossário'}")
    st.markdown(
        "Referência rápida de termos de cibersegurança usados nesta ferramenta. "
    ...
    with col_search:
        search = st.text_input(
            "🔎 Buscar termo",
            placeholder="Ex: phishing, typosquatting, entropia...",
            key="gloss_search",
        )
    with col_cat:
        cat_options = ["Todas"] + GLOSSARY_CATEGORIES
        cat_filter = st.selectbox("Categoria", cat_options, key="gloss_cat")
    ...
    if search:
        ...
        filtered = [
            t for t in filtered
            if term_lower in t["term"].lower()
            or term_lower in t["definition"].lower()
            or term_lower in t.get("example", "").lower()
        ]
    if cat_filter != "Todas":
        filtered = [t for t in filtered if t["category"] == cat_filter]

    st.caption(f"**{len(filtered)}** termos encontrados")

    if not filtered:
        st.info("Nenhum termo encontrado. Tente outra busca.")
        return

    # Renderizar termos agrupados por categoria
    categories_in_results = sorted(set(t["category"] for t in filtered))
    for cat in categories_in_results:
        cat_terms = [t for t in filtered if t["category"] == cat]
        st.subheader(f"📂 {cat}")
        for t in cat_terms:
            e = _html.escape
            with st.expander(f"**{t['term']}**", expanded=bool(search)):
                st.markdown(t["definition"])
                if t.get("example"):
                    st.markdown(
                        f'<div style="background:#1a1a2e;border-left:3px solid '
                        f'#2196F3;padding:8px 12px;border-radius:4px;margin:8px 0">'
                        f'<span style="color:#AAA;font-size:0.85em">📌 Exemplo:</span><br>'
                        f'<code style="color:#FFF">{e(t["example"])}</code></div>',
                        unsafe_allow_html=True,
                    )
                if t.get("related_module"):
                    st.caption(f"🔗 Módulo relacionado: **{t['related_module']}**")
