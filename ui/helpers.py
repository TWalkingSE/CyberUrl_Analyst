...
from utils.logger import log_analysis, setup_logger
...
# Português flexiona em gênero: o rótulo concorda com "resultado" (masculino)
# ou com "URL" (feminino), conforme o contexto de exibição.
_VERDICT_LABELS = {
    "safe": ("Seguro", "Segura"),
    "suspicious": ("Suspeito", "Suspeita"),
    "malicious": ("Malicioso", "Maliciosa"),
}


def verdict_tone(classification: str, default: str = "neutral") -> str:
    """Mapeia a classificação para o tom visual correspondente."""
    return _VERDICT_TONES.get(classification, default)


def verdict_label(classification: str, fallback: str = "",
                  lang: str | None = None, feminine: bool = False) -> str:
    """
    Rótulo traduzido da classificação.

    `feminine=True` para concordar com substantivo feminino (ex.: "URL
    Maliciosa"); o padrão concorda com masculino (ex.: "Resultado Malicioso").
    """
    forms = _VERDICT_LABELS.get(classification)
    if forms is None:
        return T(fallback, lang) if fallback else classification
    return T(forms[1] if feminine else forms[0], lang)
...
def _tone_palette(tone: str) -> tuple[str, str]:
    ...
    mapping = {
        ...
        # Nenhum chamador passa "critical" como tom hoje, mas é um valor
        # válido de `severity`; o alias evita que uma severidade usada como
        # tom caia no cinza neutro sem ninguém perceber.
...
def report_summary_fragment(report, context_label: str | None = None) -> str:
    if report is None:
        return empty_state_fragment(
            T("Nenhum resumo disponível."),
            T("Assim que uma URL for analisada, os principais indicadores aparecerão aqui."),
        )

    palette = get_palette()
    accent, background = _tone_palette(verdict_tone(report.classification))
    classification_label = verdict_label(
        report.classification, report.classification_label
    )
    stats = [
        ("Score", f"{report.score}/100"),
        (T("Fatores"), str(len(report.sections))),
        ("APIs", str(len(report.api_sections))),
        (T("Ações"), str(max(1, len(report.recommendations)))),
    ]
    cells = []
    ...
    return (
        ...
        f'<div class="kicker">{_html.escape(T("Resumo rápido"))}</div>'
        f'<h3 style="color:{accent};margin-bottom:8px;">{_html.escape(report.classification_emoji)} '
        f'{_html.escape(classification_label)} {badge_fragment(context_label or T("Último resultado"), "info")}</h3>'
        ...
        f'{_html.escape(T("Use o score como apoio visual. A decisão final continua dependendo do contexto."))}</p>'
        "</div>"
    )
...
def build_visual_breakdown_html(parts: list) -> str:
    ...
    if not parts:
        return empty_state_html(
            T("Nenhuma URL analisada."),
            T("Cole um endereço para ver a decomposição visual da URL."),
            compact=True,
        )
    html_parts = []
    for part in parts:
    ...
    return html_document(
        '<div class="panel">'
        f'<div class="kicker">{_html.escape(T("Mapa visual da URL"))}</div>'
        f'<p class="small">{_html.escape(T("Cada parte da URL aparece separada para facilitar a leitura."))}</p>'
...
def build_domain_diff(url_left: str, url_right: str) -> tuple[str, str, int, str]:
    ...
    if similarity > 80:
        message = T("Muito parecidos. Fácil confundir.")
    elif similarity > 50:
        message = T("Algumas diferenças visíveis.")
    else:
        message = T("Bastante diferentes.")
...
def _section_html(section: ReportSection) -> str:
    ...
    analogy = T(section.analogy) if section.analogy else ""
    tip = T(section.tip) if section.tip else ""
    extras = []
    if analogy:
        extras.append(
            f'<div style="color:{palette["text_muted"]};font-size:12px;margin-top:8px">&#128173; '
            f'{_html.escape(analogy)}</div>'
        )
    if tip:
        extras.append(
            f'<div style="color:{palette["accent_soft"]};font-size:12px;margin-top:8px">&#128737; '
            f'{_html.escape(tip)}</div>'
        )
    return (
        ...
        f'<div style="color:{palette["text"]};margin-top:6px">{_html.escape(content)}</div>'
        f"{''.join(extras)}"
        "</div>"
    )
...
def render_report_fragment(report) -> str:
    if report is None:
        return empty_state_fragment(
            T("Nenhum relatório disponível."),
            T("Execute uma análise para preencher este painel."),
        )
    ...
    recommendations = "".join(
        f"<li>{_html.escape(T(recommendation))}</li>" for recommendation in report.recommendations
    )
    sections_block = section_html or (
        f'<div class="empty"><p><b>{_html.escape(T("Nenhum fator detalhado."))}</b></p></div>'
    )
    api_block = api_html or (
        f'<div class="empty"><p><b>{_html.escape(T("Nenhuma consulta externa executada."))}</b></p></div>'
    )
    recommendations_block = recommendations or (
        f'<li>{_html.escape(T("Nenhuma recomendação adicional."))}</li>'
    )
    return "".join([
        f'<div class="panel" style="background:{background};border:1px solid {color};margin-bottom:12px;">',
        f'<div class="kicker">{_html.escape(T("Veredito final"))}</div>',
        f'<h2 style="margin:0;color:{color};">{_html.escape(report.classification_emoji)} ',
        f'{_html.escape(classification_label.upper())}</h2>',
        ...
        f'<div class="kicker">{_html.escape(T("Fatores analisados"))}</div>',
        sections_block,
        '</div>',
        '<div class="panel" style="margin-bottom:12px;">',
        f'<div class="kicker">{_html.escape(T("Análise externa"))}</div>',
        api_block,
        '</div>',
        '<div class="panel" style="margin-bottom:12px;">',
        f'<div class="kicker">{_html.escape(T("Recomendações"))}</div>',
        f'<ul>{recommendations_block}</ul>',
        '</div>',
        f'<p class="small">{_html.escape(T(report.disclaimer))}</p>',
    ])
...
def _apply_whois_and_ml(report, url: str, registered_domain: str, whois_result=None):
    """
    Acrescenta ao relatório as seções de WHOIS e de predição ML.

    `whois_result` deve ser o resultado já obtido pelo chamador; só é
    consultado aqui quando não vier pronto (caminho usado pelos testes).
    """
    ...
    if whois_result and whois_result.success and whois_result.age_days >= 0:
        if whois_result.is_young:
            report.sections.append(
                ReportSection(
                    title=T("Domínio jovem ({days}d)").format(days=whois_result.age_days),
                    icon="🔴",
                    severity="critical",
                    content=T("Registrado há {days} dias.").format(days=whois_result.age_days),
                    tip=T("Desconfie de domínios com menos de 30 dias."),
                )
            )
        else:
            ...
            report.sections.append(
                ReportSection(
                    title=T("Domínio estabelecido ({years}a)").format(years=years),
                    icon="✅",
                    severity="safe",
                    content=T("Registrado há aproximadamente {years} anos.").format(years=years),
                )
            )
    elif whois_result and whois_result.error:
        report.sections.append(
            ReportSection(
                title=T("WHOIS indisponível"),
                icon="ℹ️",
                severity="info",
                content=whois_result.error,
                tip=T("A análise continuou normalmente sem usar a idade do domínio."),
            )
        )
        ...
        if prediction.available:
            ...
            report.sections.append(
                ReportSection(
                    ...
                    severity=severity,
                    content=(
                        f"{T('Probabilidade maliciosa')}: "
                        f"{prediction.probability_malicious * 100:.1f}%"
                    ),
                    tip=T("Heurística + ML concordando aumenta a confiança."),
                )
            )
...
def run_analysis(raw_url: str, state=None) -> AnalysisExecutionResult:
    ...
    if not sanitized.is_valid_url:
        result.error = "\n".join(sanitized.warnings) if sanitized.warnings else T("URL inválida.")
        return result
    ...
    # Reclassifica com as constantes do config — antes eram 65/25 hardcoded,
    # que divergiriam em silêncio se os limiares mudassem.
    if analysis.score > SCORE_SUSPICIOUS_MAX:
        analysis.classification = "malicious"
        analysis.classification_label = T("Malicioso")
        analysis.classification_emoji = "🔴"
    elif analysis.score > SCORE_SAFE_MAX:
        analysis.classification = "suspicious"
        analysis.classification_label = T("Suspeito")
        analysis.classification_emoji = "🟡"
    ...
    # Repassa o WhoisResult já obtido: consultar de novo faria uma segunda
    # viagem de rede para o mesmo domínio na mesma análise.
    _apply_whois_and_ml(report, url, components.registered_domain, whois_result)
    # Registra a análise com hash SHA-256 da URL, nunca o texto puro — é a
    # garantia de privacidade que o README documenta.
    log_analysis(logger, url, report.classification, report.score)
    ...
    report.classification_label = verdict_label(
        report.classification, report.classification_label
    )
    return result
...
def query_external_apis(raw_url: str, api_keys: dict[str, str], progress_callback=None) -> ApiQueryBundle:
    ...
    if client is None:
        bundle.error = T("Módulo de APIs externas indisponível.")
        return bundle
    ...
    if progress_callback:
        progress_callback(5, T("Preparando consultas externas..."))
    ...
    if not tasks:
        bundle.error = T("Configure pelo menos uma chave de API para consultar serviços externos.")
        return bundle
    ...
    with ThreadPoolExecutor(max_workers=min(3, len(tasks))) as executor:
        ...
        for future in as_completed(futures):
            ...
            if progress_callback:
                ...
                progress_callback(percent, T("Consulta concluída: {service}").format(service=service.upper()))
